...
import { TablePicker } from './tables-panel';
...
export function OrderForm({ item, menu, onDone }: { item?: Order; menu: MenuItem[] } & Done) {
  const save = useSave(onDone);
  const [lines, setLines] = useState<{ menu_item_id: string; quantity: number }[]>(
    item?.items.map(i => ({ menu_item_id: i.menu_item_id, quantity: i.quantity })) || []
  );
  const editable = !item || item.status === 'pending';

  const dishes = menu.filter(m => m.is_active && !m.stock_ingredient_id && m.category !== 'Drinks');
  const stockItems = menu.filter(m => m.is_active && (m.stock_ingredient_id || m.category === 'Drinks'));

  const subtotal = lines.reduce((acc, line) => {
    const m = menu.find(x => x.id === line.menu_item_id);
    return acc + (Number(m?.selling_price || 0) * line.quantity);
  }, 0);
  const taxRate = Number(item?.tax_rate ?? 15);
  ...
  return (
    <form
      ...
      onSubmit={e => {
        e.preventDefault();
        const f = new FormData(e.currentTarget);
        void save.save('orders' + (item ? '/' + item.id : ''), item ? 'PUT' : 'POST', {
          items: lines,
          table_id: f.get('table_id'),
          notes: f.get('notes'),
          ...(item ? { expected_version: item.version, platform_fee: item.platform_fee || '0', delivery_cost: item.delivery_cost || '0' } : {})
        });
      }}
    >
      <TablePicker initial={item} disabled={!editable} />
      <Field label="Order instructions (optional)">
        <textarea name="notes" maxLength={2000} defaultValue={item?.notes} placeholder="Less spicy, no nuts" readOnly={!editable} />
      </Field>

      {editable && (
        <div className="form-grid">
          <Field label="Kitchen Dishes & Specials">
            <select
              value=""
              onChange={e => {
                ...
                if (id) {
                  setLines(old =>
                    old.some(i => i.menu_item_id === id)
                      ? old.map(i => i.menu_item_id === id ? { ...i, quantity: i.quantity + 1 } : i)
                      : [...old, { menu_item_id: id, quantity: 1 }]
                  );
                }
              }}
            >
              <option value="">Select kitchen dish…</option>
              {dishes.map(m => (
                <option key={m.id} value={m.id}>{m.name} · {money(m.selling_price)}</option>
              ))}
            </select>
          </Field>
          <Field label="Inventory & Drinks (Cold / Bottles)">
            <select
              value=""
              onChange={e => {
                const id = e.target.value;
      ...
      {lines.map((line, index) => {
        const m = menu.find(x => x.id === line.menu_item_id);
        const isStock = Boolean(m?.stock_ingredient_id || m?.category === 'Drinks');
        const label = item?.items.find(i => i.menu_item_id === line.menu_item_id)?.name_snapshot || m?.name || 'Menu item';
        const price = Number(m?.selling_price || 0);

        return (
          <div className="order-line" key={line.menu_item_id}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <span className={`badge ${isStock ? 'pending' : 'active'}`} style={{ fontSize: '0.72rem', padding: '1px 6px' }}>
                {isStock ? 'Inventory' : 'Kitchen'}
              </span>
              <strong>{label}</strong>
              {price > 0 && <small className="muted">({money(price)})</small>}
            </span>
            <input
              aria-label="Quantity"
              type="number"
              ...
              value={line.quantity}
              readOnly={!editable}
              onChange={e => setLines(old => old.map((l, i) => i === index ? { ...l, quantity: Number(e.target.value) } : l))}
            />
            {editable && (
              <button className="icon" type="button" aria-label="Remove dish" onClick={() => setLines(old => old.filter((_, i) => i !== index))}>
                <Trash2 size={16} />
              </button>
            )}
          </div>
      ...
      {!lines.length && <Empty>Add dishes from the kitchen or drinks from inventory.</Empty>}

      {lines.length > 0 && (
        <div style={{ background: 'rgba(0,0,0,0.02)', padding: '0.75rem 1rem', borderRadius: '8px', margin: '0.75rem 0', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', fontSize: '0.9rem' }}>
            <span className="muted">Subtotal:</span>
            <strong>{money(subtotal)}</strong>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', fontSize: '0.9rem' }}>
            <span className="muted">Estimated Table Tax ({taxRate}%):</span>
            <span>+ {money(estimatedTax)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e2e8f0', paddingTop: '0.35rem', marginTop: '0.35rem', fontSize: '1rem' }}>
            <strong>Estimated Total:</strong>
            <strong style={{ color: '#047857' }}>{money(estimatedTotal)}</strong>
          </div>
          <p className="muted small" style={{ margin: '0.35rem 0 0 0', fontSize: '0.75rem' }}>
            Tax is calculated automatically. Discounts are authorized exclusively by the cashier at billing.
          </p>
        </div>
      )}

      {editable ? (
        <Result error={save.error} busy={save.busy || !lines.length} />
      ) : (
        <p className="muted">Preparation has started. Order contents are locked.</p>
      )}
    </form>
  );
}
