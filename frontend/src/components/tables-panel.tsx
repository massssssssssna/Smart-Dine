...
export type DiningTable = { id: string; floor_id: string; name: string; seats: number; version: number };
...
function LayoutForm({ kind, item, floors, selected, onDone }: {
  ...
  return <form className="modal-form" onSubmit={async event => {
    ...
    const body = { name: form.get('name'), ...(kind === 'table' ? { floor_id: form.get('floor_id'), seats: Number(form.get('seats')) } : {}), ...(item ? { expected_version: item.version } : {}) };
    ...
    {kind === 'table' && <><Field label="Floor"><select name="floor_id" required defaultValue={(item as DiningTable)?.floor_id || selected}>{floors.map(candidate => <option key={candidate.id} value={candidate.id}>{floorLabel(candidate.name)}</option>)}</select></Field><Field label="Seats"><input name="seats" type="number" required min={1} max={100} step={1} defaultValue={(item as DiningTable)?.seats || 4} /></Field></>}
    {error && <p className="error" role="alert">{error}</p>}
